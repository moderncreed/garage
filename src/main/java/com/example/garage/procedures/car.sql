...
begin
    select  c2.id, c2.brand, f.id, f.brand, f.octane from car c2 join fuel f on f.id = c.id_fuel;
end;
...
begin
    select  c2.id, c2.brand, f.id, f.brand, f.octane from car c2 join fuel f on f.id = c.id_fuel where c.id = i;
end;